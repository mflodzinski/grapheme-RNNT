...
#SBATCH --output=slurm_rnnt_phoneme_train_%j.out
#SBATCH --error=slurm_rnnt_phoneme_train_%j.err
#SBATCH --job-name=rnnt_phoneme
...
RNNT_CONFIG="${RNNT_CONFIG:-config/config_phoneme.yaml}"
WANDB_MODE="${WANDB_MODE:-online}"
WANDB_PROJECT="${WANDB_PROJECT:-rnn-transducer}"
WANDB_NAME="${WANDB_NAME:-rnnt-phoneme-daic-${SLURM_JOB_ID:-local}}"
...
mkdir -p info_phoneme timit/outputs_phoneme "${WANDB_DIR}"
...
if [[ ! -f "${RNNT_CONFIG}" ]]; then
    echo "Config ${RNNT_CONFIG} does not exist."
    exit 1
fi
