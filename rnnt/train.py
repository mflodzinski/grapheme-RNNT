...
def progress_log_steps(total_steps: int, logs_per_epoch: int):
    if total_steps <= 0 or logs_per_epoch <= 0:
        return set()

    num_logs = min(total_steps, logs_per_epoch)
    if num_logs == 1:
        return {total_steps - 1}

    return {
        round(i * (total_steps - 1) / (num_logs - 1))
        for i in range(num_logs)
    }


def configured_logs_per_epoch(config: AttrDict):
    value = config.training.progress_logs_per_epoch
    return 5 if value is None else int(value)


def log_learning_rate(epoch, optimizer, logger, tracker=None):
    logger.info(f"-Epoch:{epoch}, Current Learning Rate:{optimizer.lr:.6f}")
...
def train(
    ...
    for step, (inputs, inputs_length, targets, targets_length) in enumerate(train_data):
        ...
        if step in log_steps:
            ...
            progress = ((step + 1) / batch_steps) * 100
            logger.info(
...
def eval(
    ...
    batch_size = getattr(eval_data, "batch_size", None)
    if batch_size is None and getattr(eval_data, "batch_sampler", None) is not None:
        batch_size = getattr(eval_data.batch_sampler, "batch_size", None)
    if max_samples is not None and batch_size:
        batch_steps = min(
            batch_steps,
            math.ceil(max_samples / batch_size),
        )
    log_steps = progress_log_steps(batch_steps, logs_per_epoch)
    seen_samples = 0
    ...
    with torch.no_grad():
        for step, (inputs, inputs_length, targets, targets_length) in enumerate(eval_data):
            ...
            should_stop = max_samples is not None and seen_samples >= max_samples
            if step in log_steps or should_stop:
                if max_samples is None:
                    process = ((step + 1) / batch_steps) * 100
                else:
                    process = (seen_samples / max_samples) * 100
...
def train_model(
    ...
    min_delta = (
        config.training.early_stopping_min_cer_delta
        if config.training.early_stopping_min_cer_delta is not None
        else config.training.early_stopping_min_delta or 0.0
    )
    restore_best = (
        True
    ...
    for epoch in range(config.training.epochs):
        ...
        if should_evaluate:
            ...
            if is_improved(val_cer, best_val_cer, min_delta):
        ...
        log_learning_rate(epoch, optimizer, logger, tracker)
